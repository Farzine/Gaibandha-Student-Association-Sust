...
//       toast.success(`Member request for ${user.name} approved successfully!`);
//     } catch (err: any) {
//       if (err.response) {
//         // err.response.status is 400 if ValidationError
//         // The server is sending: { success: false, message: "Validation failed", errors: {...} }
...
//         }
//       } else {
//         // No response object, e.g. network error
...
//       // remove from UI
//       setRequests((prev) => prev.filter((r) => r._id !== user._id));
//       toast.success(`Member request for ${user.name} rejected and user deleted!`);
//     } catch (err: any) {
//       // This is where we catch the 400 response with validationErrors
//       if (err.response) {
//         // err.response.status is 400 if ValidationError
//         // The server is sending: { success: false, message: "Validation failed", errors: {...} }
//         const { status, data } = err.response;
...
//         if (status === 400 && data.errors) {
//           // data.errors will be something like { linkedinId: "linkedinId is not a valid LinkedIn URL!" }
//           // You could display each error in a toast, or combine them
//           Object.values(data.errors).forEach((msg: any) => {
//             toast.error(msg);
//           });
//         } else {
